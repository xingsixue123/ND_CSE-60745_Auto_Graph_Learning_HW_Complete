...
resource allocation, owning a paged address space together with whatever port rights it
holds; a \emph{thread} is the unit of CPU utilisation, so that a UNIX process is simply a
task with a single thread; a
...
is fixed at boot as a power-of-two multiple of the machine-dependent one, itself 512 bytes on
the VAX --- letting the same
...
precisely the claim the paper does not test.

History then split the verdict. The mechanisms
...
Two of its assumptions have since expired outright. Network-transparent IPC
...
about 3.1k cycles across 32 cores where naive broadcast and unicast climb to roughly 13.8k
and 12.7k respectively;
...
it, so what is measured is specialisation rather than architecture.
...
does not arrive until somewhere around thirteen or fourteen cores. The messaging table quietly reinforces
the point, showing URPC at 450 cycles of latency against 424 for L4's decade-older IPC, the
real gains again being throughput and a much smaller cache footprint rather than latency.

The hardware bet was half right. Cache
...
decade. Their standards of proof move in the opposite direction to their confidence: 1965
offers a design prospectus
