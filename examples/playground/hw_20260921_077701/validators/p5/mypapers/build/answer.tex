\section*{Ex.~2.7 --- Three papers from the ``Scheduling'' group}
...
iterations/s ($2.01\!:\!1$), an $8\!:\!3\!:\!1$ database allocation reproduced in response time as
well as throughput ($17.19$, $43.19$ and $132.20$\,s), and currencies insulating one user's
inflation from another (aggregate ratio $1.01\!:\!1$ before, $1.00\!:\!1$ after).

\paragraph{Critical judgment.}
The evaluation's soft spot is one the paper half-concedes. Individual 60-second runs scatter
badly, a $10\!:\!1$ allocation being observed at $13.42\!:\!1$, and the authors' reply is that
large ratios ``converge toward their allocated values over longer time intervals'', offering a
$20\!:\!1$ allocation that averaged $19.08\!:\!1$ over three minutes. That answer costs them
their framing: the introduction argues interactive systems need control ``at a time scale of
milliseconds to seconds'', and the related-work section dismisses fair-share schedulers precisely
for delivering fairness only ``on a time scale of minutes''. Randomisation makes the error fall
only as $1/\sqrt{n}$, their own coefficient of variation $\sqrt{(1-p)/np}$, so on the timescale
the paper says matters it supplies roughly what it criticises.
...
inflation safe, yet in the measured artifact they are unprotected: the user commands are setuid
root, and the access-control lists deciding who may inflate a currency are proposed but left as
what ``a complete lottery scheduling system should'' provide.
...
\paragraph{Critical judgment.}
The method is asymmetric in a way the paper notices locally and forgets globally. A
counterexample refutes ``RM always wins''; it cannot establish ``EDF wins'', and Buttazzo says
exactly that of his jitter example, which ``does not prove that EDF always introduces less
jitter'', yet the abstract and conclusions generalise anyway. The overhead argument is the
weakest link: Section 3 counts preemptions, never measures a context switch on real hardware, and
never nets the saved switches against EDF's per-job deadline update, so the verdict rests on a
proxy measured over synthetic task sets. Two things that would decide the industrial argument are
absent: multiprocessors, and the certification and tooling ecosystem built on fixed priorities,
which he reduces to a circularity.

He does not, however, stack the deck. He grants RM the $O(1)$ ready queue and the cheaper exact
test when deadlines are shorter than periods, grants that RM confines an overrun to tasks of
lower priority while EDF confines it nowhere, and concludes both that which overload behaviour is
preferable is ``highly application dependent'' and that the two algorithms are ``not very well
suited'' to overload at all, the remedy being resource reservation. That concession aged best:
Linux's
...
\medskip
All three argue the same way: 2-4 unseats its own predecessor's randomness, and 2-1 unseats
static rate priorities, each on the ground that a scheduler should carry the extra state needed
