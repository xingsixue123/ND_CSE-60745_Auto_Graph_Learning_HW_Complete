...
class LinkPredictor:
    ...
    def fit(self, Z, pairs_train, y_train, pairs_val, y_val):
        ...
        for C in self.C_GRID:                      # model selection on validation MRR
            ...
            # rank of the positive within its query (ties counted optimistically; the
            # reported metrics come from the scaffolding's evaluate_ranking, which breaks
            # ties at random -- this is only the internal C-selection criterion)
