\section*{Exercise 1.5 -- 1.8}
\label{p2:sec:main}

\subsection*{Ex. 1.5 (5 pts) --- What a compiler may assume about loading}
\label{p2:sec:loadassumptions}
...
\subsection*{Ex. 1.6 (5 pts) --- From double-click to window on screen}
\label{p2:sec:launch}

The two cases differ only in how the command line is assembled. A double-click makes
the desktop environment look up the icon's file association to recover an executable
path and arguments; a typed command makes the shell tokenise the line, expand variables
and globs, and resolve the name against \texttt{PATH}. Once a path and an argument
vector exist, the rest is identical.

The shell or launcher calls \texttt{fork()}, giving a child whose address space is
shared copy-on-write, and the child calls \texttt{exec()} on the resolved path. The
kernel checks the execute permission, opens the file, reads its header and validates
the magic number, which identifies the format and selects the loader; a bad magic
number fails the exec rather than running garbage. The child's old address space is
torn down and a new one built: text, initialised data, zero-filled BSS, heap and stack,
with \texttt{argv} and \texttt{envp} copied onto the new stack. The segments are
memory-mapped rather than read in, so almost no I/O happens yet. If the binary is
dynamically linked, the interpreter named in its header (\texttt{ld.so}) is mapped and
runs first, mapping the shared libraries and resolving symbols. The kernel then updates
the process control block (page tables, inherited file descriptors, reset signal
handlers) and puts the process on the ready queue. When the scheduler dispatches it,
execution begins at the entry point and pages are faulted in as they are touched.
Startup code initialises the language runtime and calls \texttt{main()}, which connects
to the window system, creates a window and draws into it; the compositor puts those
pixels on the screen.


\subsection*{Ex. 1.7 (5 pts) --- Virtual to physical under contiguous allocation}
\label{p2:sec:translate}
...
\subsection*{Ex. 1.8 (5 pts) --- The round-robin scheduler}
\label{p2:sec:rr}
