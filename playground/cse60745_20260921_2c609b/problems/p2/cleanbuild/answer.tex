...
\paragraph{Freebase.} \emph{The name denotes two unrelated graphs.} HGB's Freebase is a general
knowledge graph: 8 node types, 36 relations, 180{,}098 nodes, target type \textsc{book} with 7
classes, and \textbf{no node features whatsoever} --- so it isolates how much a model can extract
from pure typed structure. It is book-\emph{centric} only in the sense that \textsc{book} is the
labelled type; its schema is not a star but is in fact \emph{complete}
(Figure~\ref{p2:fig:schemasc}). HeCo's ``Freebase'' is instead a \emph{movie}
...
\paragraph{Amazon (e-commerce) --- heterogeneous in $\mathcal{R}$ only.} HGB's Amazon is the
limiting case of the definition in Section~1, and worth stating for that reason. It has
\textbf{one} node type (\emph{product}, 10{,}099 nodes) and \textbf{two} relation types,
\emph{co-viewing} and \emph{co-purchasing}, over 148{,}659 edges --- the electronics subset
preprocessed by GATNE (Cen et al., KDD 2019). So $|\mathcal{A}|=1$ but $|\mathcal{R}|=2$, giving
$|\mathcal{A}|+|\mathcal{R}|=3>2$: it is a heterogeneous network even though every node is the
same type, which is precisely why the condition in Section~1 is stated as a sum rather than as
``more than one node type''. Its network schema is therefore a single node carrying two distinct
self-relations. It is used for \textbf{link prediction only} (it has no node labels), scored with
ROC-AUC and MRR. It is also the cleanest illustration of the limit of meta-path methods: with one
node type there is no type alternation to define a meta-path over, and HGB's link-prediction
table accordingly leaves MAGNN's Amazon entry blank, noting that vacancies are ``due to lack of
meta-paths on those datasets''. Relation-aware but meta-path-free models (R-GCN, GATNE,
simple-HGN) are the ones that can run on it at all.
...
\toprule
Dataset & Version & Node counts & Note \\
\midrule
\multirow{3}{*}{DBLP}
 & HAN   & paper 14{,}328 / 14{,}327, author 4{,}057, term \textbf{8{,}789}, venue 20
         & P--T $=88{,}420$ \\
 & MAGNN / HeCo / HGB & paper 14{,}328, author 4{,}057, term \textbf{7{,}723}, venue 20
         & P--T $=85{,}810$ \\
 & GTN   & 18{,}405 total (= paper + author + venue; \emph{term dropped}) & 4 edge types \\
\midrule
\multirow{3}{*}{ACM}
 & HAN   & paper 3{,}025, author 5{,}835, subject 56 & P--A $=9{,}744$ \\
 & HGB   & paper 3{,}025, author \textbf{5{,}959}, subject 56, term 1{,}902 & P--A $=9{,}949$ \\
 & HeCo  & paper \textbf{4{,}019}, author \textbf{7{,}167}, subject \textbf{60} & P--A $=13{,}407$ \\
\midrule
\multirow{3}{*}{IMDB}
 & HAN   & movie 4{,}780, director 2{,}269, actor 5{,}841 & \\
 & MAGNN & movie 4{,}278, director 2{,}081, actor 5{,}257 & 3 classes, single-label \\
 & HGB   & movie 4{,}932, director 2{,}393, actor 6{,}124, keyword 7{,}971
...
Each schema in Figures~\ref{p2:fig:schemas} and~\ref{p2:fig:schemasc} is
$T_G=(\mathcal{A},\mathcal{R})$ for one benchmark family: circles are node types (letter = the
initial used in meta-path notation), edges are relation types. Inverse relations are omitted for
readability in Figure~\ref{p2:fig:schemas}; Freebase (Figure~\ref{p2:fig:schemasc}) has no
inverse relations at all and is drawn complete.

A schema alone does not say which compositions of relations the literature actually uses. The
meta-paths conventionally instantiated on each of these schemas, together with the semantic
similarity each one encodes, are collected in Table~\ref{p2:tab:metapaths}; these are the paths
that HAN, MAGNN and HERec take as input, and reading them against the schemas above shows how a
two-hop alternation such as $A\!-\!P\!-\!A$ turns a typed bipartite relation into a homogeneous
similarity over the target type.
...
\end{minipage}
\caption{Network schemas $T_G=(\mathcal{A},\mathcal{R})$ of the bibliographic, media, large-scale
and review benchmarks. Circles are node types (the letter is the initial used in meta-path
notation); labelled edges are relation types. Inverse relations are omitted in this figure ---
but see Figure~\ref{p2:fig:schemasc}, where nothing is omitted.}
...
\begin{tikzpicture}[scale=0.92, every node/.style={transform shape}, label distance=9.5mm]
  % positions first, so the 28 chords can be drawn underneath the type circles
  \path (90:2.75)  coordinate (B)  (45:2.75)  coordinate (F)
        (0:2.75)   coordinate (M)  (-45:2.75) coordinate (S)
        (-90:2.75) coordinate (Pe) (-135:2.75)coordinate (L)
        (180:2.75) coordinate (O)  (135:2.75) coordinate (Bu);
  % all C(8,2)=28 pairwise relations, drawn first so the circles sit on top
  \foreach \xa in {B,F,M,S,Pe,L,O,Bu}{
    \foreach \ya in {B,F,M,S,Pe,L,O,Bu}{
      \draw[draw=black!28, line width=0.4pt] (\xa) -- (\ya);}}
  % type circles
  \node[p2ent, fill=\pTwoTgt, label={[p2lab]90:Book}]          (nB)  at (B)  {B};
  \node[p2ent, fill=\pTwoAgt, label={[p2lab]45:Film}]          (nF)  at (F)  {F};
  \node[p2ent, fill=\pTwoAtt, label={[p2lab]0:Music}]          (nM)  at (M)  {M};
  \node[p2ent, fill=\pTwoAgt, label={[p2lab]-45:Sports}]       (nS)  at (S)  {S};
  \node[p2ent, fill=\pTwoVen, label={[p2lab]-90:People}]       (nPe) at (Pe) {P};
  \node[p2ent, fill=\pTwoVen, label={[p2lab]-135:Location}]    (nL)  at (L)  {L};
  \node[p2ent, fill=\pTwoVen, label={[p2lab]180:Organization}] (nO)  at (O)  {O};
  \node[p2ent, fill=\pTwoAtt, label={[p2lab]135:Business}]     (nBu) at (Bu) {Bu};
  % one self-relation per type, looping radially outward from each circle
  \foreach \nd/\oa/\ia in {nB/116/64, nF/71/19, nM/26/-26, nS/-19/-71,
                           nPe/-64/-116, nL/-109/-161, nO/206/154, nBu/161/109}{
    \draw[draw=black!55, line width=0.5pt]
      (\nd) to[out=\oa,in=\ia,looseness=6] (\nd);}
\end{tikzpicture}
\\[4pt]{\small\textbf{Freebase (HGB)} --- the schema is \emph{complete}:
$\binom{8}{2}=28$ pairwise relations $+$ one self-relation per type $=36$.}
\end{minipage}
\caption{Network schema of Freebase (HGB), drawn \emph{in full} --- all 36 relation types are
shown, so unlike Figure~\ref{p2:fig:schemas} nothing is omitted here.
Freebase is the one benchmark whose schema is \textbf{complete}: every one of the
$\binom{8}{2}=28$ unordered type pairs carries exactly one relation, and every type carries one
self-relation, giving $28+8=36$. Note the contrast with DBLP/ACM/IMDB: \textbf{none of these 36
are inverses} --- all 36 are distinct forward relations, so this count is \emph{not} inflated by
the forward/inverse doubling of Section~1, whereas DBLP's ``6 edge types'' is 3 relations plus
their 3 inverses. \textsc{book} (shaded) is the labelled target type; the largest single relation
is \textsc{music}--\textsc{music}, 283{,}670 edges or $27\%$ of the graph.}
\label{p2:fig:schemasc}
\end{figure}

The remaining benchmarks described in Section~2 are drawn in Figure~\ref{p2:fig:schemasd}. Two of
them are instructive rather than merely additional. \textbf{Amazon} is the degenerate case that
justifies writing the condition of Section~1 as $|\mathcal{A}|+|\mathcal{R}|>2$ rather than
``more than one node type'': its schema is a \emph{single} circle carrying two distinct
self-relations, so it is heterogeneous in $\mathcal{R}$ alone. It is also why no meta-path can be
defined on it --- with one node type there is no type alternation to alternate over, which is the
structural reason the meta-path-based models have a blank entry on it. \textbf{PubMed (HGB)} is
the opposite extreme among the link-prediction sets, and is the second benchmark here whose
schema is \emph{complete}: all $\binom{4}{2}=6$ unordered type pairs carry a relation and all
four types carry a self-relation, $6+4=10$, matching its published count of 10 relation types.
Its prediction target, \textsc{disease}--\textsc{disease}, is itself one of those
self-relations --- so unlike DBLP or ACM the task is to predict edges \emph{within} a single
type, which is why the type-alternation machinery of meta-paths buys so little on it.
...
\begin{tikzpicture}[scale=0.92, every node/.style={transform shape}]
  \node[p2ent, fill=\pTwoTgt, label={[p2lab]below:Paper}]  (P) at (0,0) {P};
  \node[p2ent, fill=\pTwoAgt, label={[p2lab]below:Author}] (A) at (-3.0,0) {A};
  \node[p2ent, fill=\pTwoVen, label={[p2lab]below:Venue}]  (V) at (3.2,0) {V};
  \draw[p2rel] (A) -- node[p2el]{writes} (P);
  \draw[p2rel] (P) -- node[p2el]{published in} (V);
  \draw[p2rel] (P) to[out=120,in=60,looseness=9] node[p2el,above]{references} (P);
\end{tikzpicture}
\\[2pt]{\small\textbf{AMiner}}
\end{minipage}\hfill
\begin{minipage}{0.48\textwidth}\centering
\begin{tikzpicture}[scale=0.92, every node/.style={transform shape}]
  \node[p2ent, fill=\pTwoTgt, label={[p2lab]-35:Movie}]      (M) at (0,0) {M};
  \node[p2ent, fill=\pTwoAgt, label={[p2lab]below:User}]     (U) at (-3.0,0) {U};
  \node[p2ent, fill=\pTwoVen, label={[p2lab]below:Director}] (D) at (3.2,0) {D};
  \node[p2ent, fill=\pTwoAgt, label={[p2lab]above:Actor}]    (A) at (0,1.9) {A};
  \node[p2ent, fill=\pTwoAtt, label={[p2lab]below:Type}]     (T) at (0,-1.9) {T};
  \draw[p2rel] (U) -- node[p2el]{rates} (M);
  \draw[p2rel] (M) -- node[p2el]{directed by} (D);
  \draw[p2rel] (M) -- node[p2el]{stars} (A);
  \draw[p2rel] (M) -- node[p2el]{has} (T);
\end{tikzpicture}
\\[2pt]{\small\textbf{Douban Movie}}
\end{minipage}

\vspace{12pt}
\begin{minipage}{0.48\textwidth}\centering
\begin{tikzpicture}[scale=0.92, every node/.style={transform shape}]
  \node[p2ent, fill=\pTwoAgt, label={[p2lab]above:User}]       (U) at (-1.7,0) {U};
  \node[p2ent, fill=\pTwoTgt, label={[p2lab]above:Movie}]      (M) at (1.7,0) {M};
  \node[p2ent, fill=\pTwoAtt, label={[p2lab]below:Occupation}] (O) at (-1.7,-1.9) {O};
  \node[p2ent, fill=\pTwoVen, label={[p2lab]below:Genre}]      (G) at (1.7,-1.9) {G};
  \draw[p2rel] (U) -- node[p2el]{rates} (M);
  \draw[p2rel] (U) -- node[p2el,rotate=90]{has} (O);
  \draw[p2rel] (M) -- node[p2el,rotate=90]{has} (G);
\end{tikzpicture}
\\[2pt]{\small\textbf{MovieLens / ML-100K}}
\end{minipage}\hfill
\begin{minipage}{0.48\textwidth}\centering
\begin{tikzpicture}[scale=0.92, every node/.style={transform shape}]
  \node[p2ent, fill=\pTwoTgt, label={[p2lab]below:Product}] (Pr) at (0,-0.35) {Pr};
  \draw[p2rel] (Pr) to[out=155,in=205,looseness=9] node[p2el,left]{co-viewed} (Pr);
  \draw[p2rel] (Pr) to[out=25,in=-25,looseness=9] node[p2el,right]{co-purchased} (Pr);
\end{tikzpicture}
\\[2pt]{\small\textbf{Amazon} --- $|\mathcal{A}|=1$, $|\mathcal{R}|=2$}
\end{minipage}

\vspace{14pt}
\begin{minipage}{\textwidth}\centering
\begin{tikzpicture}[scale=0.95, every node/.style={transform shape}]
  % --- the 6 pairwise relations, drawn before the circles so they pass underneath
  \path (-3.1,1.25) coordinate (cC) (3.1,1.25) coordinate (cG)
        (-3.1,-1.25) coordinate (cS) (3.1,-1.25) coordinate (cD);
  \draw[p2rel] (cC) -- node[p2el,pos=0.5]{in} (cG);
  \draw[p2rel] (cS) -- node[p2el,pos=0.5]{with} (cD);
  \draw[p2rel] (cC) -- node[p2el,pos=0.5]{in} (cS);
  \draw[p2rel] (cG) -- node[p2el,pos=0.5]{causing} (cD);
  \draw[p2rel] (cC) -- node[p2el,pos=0.26]{in} (cD);
  \draw[p2rel] (cG) -- node[p2el,pos=0.26]{with} (cS);
  % --- type circles (names set to the outer side, clear of the self-loops above/below)
  \node[p2ent, fill=\pTwoAtt, label={[p2lab]180:Chemical}] (C) at (cC) {C};
  \node[p2ent, fill=\pTwoAgt, label={[p2lab]0:Gene}]       (G) at (cG) {G};
  \node[p2ent, fill=\pTwoVen, label={[p2lab]180:Species}]  (S) at (cS) {S};
  \node[p2ent, fill=\pTwoTgt, label={[p2lab]0:Disease}]    (D) at (cD) {D};
  % --- the 4 self-relations: straight up from the top row, straight down from the bottom
  \draw[p2rel] (C) to[out=115,in=65,looseness=7]   node[p2el,above]{and} (C);
  \draw[p2rel] (G) to[out=115,in=65,looseness=7]   node[p2el,above]{and} (G);
  \draw[p2rel] (S) to[out=-65,in=-115,looseness=7] node[p2el,below]{and} (S);
  % the LP target relation, emphasised
  \draw[p2rel, line width=1.5pt, draw=orange!75!black]
        (D) to[out=-65,in=-115,looseness=7] node[p2el,below]{\textbf{and}} (D);
\end{tikzpicture}
\\[4pt]{\small\textbf{PubMed (HGB)} --- the schema is \emph{complete}:
$\binom{4}{2}=6$ pairwise relations $+$ one self-relation per type $=10$.
The bold loop is the link-prediction target, \textsc{disease}--\textsc{disease}.}
\end{minipage}
\caption{Network schemas of the five benchmarks described in Section~2 but not drawn in
Figures~\ref{p2:fig:schemas}--\ref{p2:fig:schemasc}. Conventions as before; inverse relations are
omitted. AMiner is drawn in the \mbox{metapath2vec} form (author/paper/venue), with the
paper--paper reference relation that the HeCo subset also carries. Douban Movie and
MovieLens/ML-100K are user--item HINs: the user--item rating edge is the prediction target, and
the remaining types are side information attached to one side or the other. Amazon has a single
node type, so its whole schema is one circle with two self-relations. PubMed is drawn
\emph{in full}: its four types are \textsc{gene}, \textsc{disease}, \textsc{chemical},
\textsc{species} and its ten relations are \textsc{gene-and-gene},
\textsc{gene-causing-disease}, \textsc{disease-and-disease}, \textsc{chemical-in-gene},
\textsc{chemical-in-disease}, \textsc{chemical-and-chemical}, \textsc{chemical-in-species},
\textsc{species-with-gene}, \textsc{species-with-disease} and \textsc{species-and-species}
(edge labels abbreviate the verb); these are taken verbatim from the dataset's own
\texttt{info.dat}, and their counts sum to the published $244{,}986$.}
\label{p2:fig:schemasd}
\end{figure}
...
Which metric is ``correct'' is not a property of the task in the abstract but of the
dataset--task pair, because it is the dataset that fixes the target node type, whether the labels
are single- or multi-label, and how many classes there are.
Table~\ref{p2:tab:summary} therefore closes the section by mapping each benchmark surveyed in
Section~2 to its target type, its task, and the metrics that paper after paper actually reports on
it. Two entries in it are worth singling out: the OGB datasets break the Macro-/Micro-F1 habit of
the HIN literature and are scored by the leaderboard on plain \textbf{accuracy}, and the
recommendation-flavoured datasets (Yelp, Douban) are scored on \textbf{MAE/RMSE} because their
task is rating prediction rather than classification.
...
Yelp / Douban & review    & user--item & LP / rating & --- & Prec@K, Rec@K; MAE, RMSE \\
