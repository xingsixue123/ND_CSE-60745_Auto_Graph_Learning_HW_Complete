...
Each schema in Figures~\ref{p2:fig:schemas}--\ref{p2:fig:schemasc} is
$T_G=(\mathcal{A},\mathcal{R})$ for one benchmark family: circles are node types (letter = the
initial used in meta-path notation), edges are relation types. Inverse relations are omitted
for readability.
...
\end{minipage}
\caption{Network schemas $T_G=(\mathcal{A},\mathcal{R})$ of the bibliographic and media
...
\end{minipage}
\caption{Network schemas of the large-scale bibliographic and review benchmarks.
Conventions as in Figure~\ref{p2:fig:schemas}.}
\label{p2:fig:schemasb}
\end{figure}
...
\end{minipage}
\caption{Network schema of Freebase (HGB). It is drawn as a star because \textsc{book} is the
labelled target type and is adjacent to every other type. Conventions as in
Figure~\ref{p2:fig:schemas}.}
