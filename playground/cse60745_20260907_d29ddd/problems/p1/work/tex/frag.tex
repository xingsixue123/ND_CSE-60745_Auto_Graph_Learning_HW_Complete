...
edges listed above:
\texttt{G = nx.Graph(); G.add\_nodes\_from(range(10));
...
\emph{Petersen graph} (verified with
\texttt{nx.is\_isomorphic(G, nx.petersen\_graph())}, which returns
\texttt{True}), which is the standard $3$-regular, girth-$5$ graph on ten
